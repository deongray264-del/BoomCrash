...
v3 ADDITIONS (2026-07-01) — BOOM 500/1000 & CRASH 500/1000 support:
─────────────────────────────────────────────────────────────────
BOOM/CRASH indices are NOT a random walk like R_/1HZ symbols — they're a
steady background drift punctuated by engineered spikes (BOOM spikes UP,
CRASH spikes DOWN) at a roughly-known average tick interval baked into the
symbol name (500 or 1000). The v1/v2 layer stack (Hurst/HMM/GARCH/etc.) is
tuned to find edge in genuine random walks and would largely miss this
mechanic, so BOOM/CRASH gets its own dedicated calibration + feature layer
on top of (not instead of) the existing 18 layers:

  - fetch_boom_crash_symbols() discovers the 4 symbols live from
    active_symbols (no hardcoded symbol codes) and verifies CALL/PUT support.
  - BOOM/CRASH gets a longer bootstrap history (25,000 vs 10,000 ticks) and
    larger live buffer, so even the 1000-family symbols show ~25-50 observed
    spikes to fit a gap distribution against, not just a handful.
  - fit_boom_crash_model() detects historical spike ticks (robust MAD
    threshold, iteratively refit so big spikes don't mask smaller ones) and
    fits: mean/std ticks-between-spikes, background drift+vol between spikes,
    ticks-since-last-spike.
  - boom_crash_hazard() converts "ticks since last spike" + a candidate
    contract duration into P(a spike lands inside this trade), modeling the
    gap distribution as Normal(mean_gap, std_gap).
  - This hazard model drives THREE things: (1) a new fusion evidence term
    that trades WITH the background drift, strength scaled by how far we are
    from the next expected spike; (2) a soft dampener that shrinks confidence
    as hazard rises; (3) a hard gate that skips the trade outright if
    P(spike within the chosen duration) > BC_MAX_HAZARD_GATE — no amount of
    drift edge survives an unresolved spike mid-contract.
  - Fitting runs inside fit_symbol_models(), so it rides the EXISTING
    calibration cadence (once at startup, every 2 hours, and on loss
    triggers) — no separate calibration system to maintain.
  - The FIX-5 direction-balance safety net is skipped for BOOM/CRASH: a BOOM
    index trading mostly PUT (and CRASH mostly CALL) between spikes is the
    index's correct engineered behavior, not a bug to correct away.
  - v3 ships Rise/Fall only. Deriv's "Only Ups"/"Only Downs" contract type
    (built specifically around this drift mechanic) is a natural v4 addition
    once this layer is validated live, but needs its own buy/settlement
    wiring and is deliberately deferred to keep this change lower-risk.
...
# ── BOOM/CRASH spike-index calibration ──────────────────────────────────────
# BOOM/CRASH indices are NOT a random walk like R_/1HZ symbols. They are a
# steady background drift punctuated by engineered spikes: BOOM spikes UP,
# CRASH spikes DOWN, on average once every N ticks (500 or 1000, per the
# symbol name). The dominant tradeable signal is "how close are we to the
# next expected spike" (hazard), not Hurst/HMM/GARCH noise-hunting — those
# 18 layers still run and still add confirmation value, but the BOOM/CRASH
# hazard+drift layer is given a strong default weight because it reflects
# the index's actual engineered mechanic rather than a statistical guess.
BC_HISTORY_BOOTSTRAP_COUNT = 25000     # ticks fetched at startup for BOOM/CRASH
                                        # symbols — more than R_/1HZ so the spike-
                                        # gap distribution has enough samples even
                                        # for the 1000-family (~25-50 spikes seen)
BC_MAXLEN            = 25000           # live rolling tick buffer, same reasoning
BC_MIN_SPIKES_FOR_FIT = 3              # below this, fall back to the nominal
                                        # 500/1000 interval baked into the name
BC_SPIKE_MAD_K        = 6.0            # robust MAD multiple that defines a
                                        # "spike" tick vs background noise
BC_MAX_HAZARD_GATE    = 0.55           # skip the trade if P(spike lands inside
                                        # the chosen contract duration) exceeds this
BC_HAZARD_DAMPEN_FROM = 0.35           # above this hazard, start damping the
                                        # background-drift evidence in fusion
...
class SymbolModels:
    ...
    # BOOM/CRASH spike-hazard calibration profile (None for R_/1HZ symbols)
    boom_crash: Optional[dict] = None
...
async def fetch_boom_crash_symbols(client):
    """Fetches the BOOM/CRASH 500 & 1000 spike-index symbols. Discovers the
    exact symbol codes live from active_symbols rather than hardcoding them
    (Deriv's symbol codes/suffixes have changed before — see the R_ symbol
    handling above), then verifies CALL/PUT support via contracts_for, same
    pattern as fetch_tradable_symbols()."""
    resp = await client.send({"active_symbols": "brief"})
    if "error" in resp:
        print(f"[fetch_boom_crash_symbols] active_symbols error: {resp['error']}")
        return []
    ...
    for s in resp.get("active_symbols", []):
        ...
        if not symbol:
            continue
        name_up = symbol.upper()
        if not (name_up.startswith("BOOM") or name_up.startswith("CRASH")):
            continue
        if not ("500" in name_up or "1000" in name_up):
            continue
        if s.get("market") != "synthetic_index":
            continue
        if not s.get("exchange_is_open", 1):
            continue
        candidates.append(symbol)
    print(f"[fetch_boom_crash_symbols] {len(candidates)} BOOM/CRASH candidates: {candidates}")
    ...
    if cf_errors:
        print(f"[fetch_boom_crash_symbols] {len(cf_errors)} contracts_for calls failed, e.g.: {cf_errors[:3]}")
    print(f"[fetch_boom_crash_symbols] verified BOOM/CRASH symbols: {verified}")
    return verified
...
# ---------------------------------------------------------------------------
# ---------------------------------------------------------------------------
# BOOM/CRASH SPIKE-HAZARD MODEL
# BOOM/CRASH indices are a compound process: a steady background drift
# (opposite the spike direction) punctuated by engineered spikes at a
# roughly-known average tick interval (500 or 1000, from the symbol name).
# This is NOT the same statistical animal as R_/1HZ random-walk symbols, so
# it gets its own dedicated calibration + feature layer rather than trying
# to force the Hurst/HMM/GARCH stack to find a signal that isn't there.
# ---------------------------------------------------------------------------
...
def boom_crash_spike_dir(symbol: str) -> int:
    """+1 = BOOM (spikes UP), -1 = CRASH (spikes DOWN)."""
    return 1 if symbol.upper().startswith("BOOM") else -1


def boom_crash_nominal_interval(symbol: str) -> int:
    """The engineered average tick interval baked into the symbol name."""
    return 1000 if "1000" in symbol else 500


def detect_spike_ticks(returns: np.ndarray, spike_dir: int, k_mad: float = BC_SPIKE_MAD_K) -> np.ndarray:
    """Identify indices of engineered spike ticks: an outsized move in the
    family's fixed spike direction. Uses an iteratively-robust MAD threshold
    on the background distribution so a few huge spikes don't inflate the
    baseline and mask smaller genuine spikes."""
    if len(returns) < 50:
    ...
    for _ in range(3):
        ...
        z = (r - med) / (1.4826 * mad)
        candidate = (z * spike_dir) > k_mad
        mask = ~candidate
    spike_idx = np.where((z * spike_dir) > k_mad)[0]
    return spike_idx


def fit_boom_crash_model(sd) -> Optional[dict]:
    """Fits the spike-gap distribution and background drift for a BOOM/CRASH
    symbol. Returns None for non-BOOM/CRASH symbols (cheap no-op) so this can
    be called unconditionally from fit_symbol_models() for every symbol."""
    symbol = sd.symbol
    if not is_boom_crash_symbol(symbol):
        return None
    returns = sd.returns()
    nominal = boom_crash_nominal_interval(symbol)
    if len(returns) < 300:
        return None
    spike_dir = boom_crash_spike_dir(symbol)
    spike_idx = detect_spike_ticks(returns, spike_dir)

    if len(spike_idx) >= BC_MIN_SPIKES_FOR_FIT:
        gaps = np.diff(np.concatenate([[-1], spike_idx]))
        gaps = gaps[gaps > 0]
        mean_gap = float(np.mean(gaps))
        std_gap  = float(np.std(gaps)) if len(gaps) > 1 else mean_gap * 0.3
    else:
        # Not enough observed spikes yet — fall back to the nominal interval
        # baked into the symbol name rather than an unreliable tiny sample.
        mean_gap = float(nominal)
        std_gap  = float(nominal) * 0.3

    bg_mask = np.ones(len(returns), dtype=bool)
    if len(spike_idx):
        bg_mask[spike_idx] = False
    bg_returns = returns[bg_mask]
    bg_drift = float(np.mean(bg_returns)) if len(bg_returns) else 0.0
    bg_std   = float(np.std(bg_returns))  if len(bg_returns) else 1e-6

    ticks_since_last = int(len(returns) - 1 - spike_idx[-1]) if len(spike_idx) else len(returns)
    spike_mags = np.abs(returns[spike_idx]) if len(spike_idx) else np.array([])

    return {
        "spike_dir":         spike_dir,
        "nominal_interval":  nominal,
        "mean_gap":          mean_gap,
        "std_gap":           max(std_gap, mean_gap * 0.1),
        "n_spikes_seen":     int(len(spike_idx)),
        "bg_drift":          bg_drift,     # steady per-tick drift, opposite spike_dir
        "bg_std":            bg_std,
        "ticks_since_last":  ticks_since_last,
        "spike_mag_mean":    float(np.mean(spike_mags)) if len(spike_mags) else 0.0,
        "fitted_at":         time.time(),
    }


def boom_crash_hazard(model: dict, ticks_since_last: int, duration_ticks: float) -> float:
    """P(a spike lands within the next `duration_ticks` ticks), given
    `ticks_since_last` ticks have already elapsed with no spike. Models the
    inter-spike gap as Normal(mean_gap, std_gap) and computes the conditional
    probability the gap falls in (ticks_since_last, ticks_since_last+duration],
    given it has already survived past ticks_since_last."""
    mu, sigma = model["mean_gap"], model["std_gap"]
    survive_now  = float(norm.sf(ticks_since_last, mu, sigma))
    survive_next = float(norm.sf(ticks_since_last + duration_ticks, mu, sigma))
    if survive_now < 1e-6:
        return 1.0   # already well past the expected interval — a spike is overdue
    return float(np.clip((survive_now - survive_next) / survive_now, 0.0, 1.0))


def compute_boom_crash_features(sd, model: Optional[dict]) -> Optional[dict]:
    """Live per-tick BOOM/CRASH features: re-scans only the recent tail (cheap)
    to get an up-to-date ticks-since-last-spike count rather than relying on
    the potentially-stale count from the last calibration pass."""
    if model is None:
        return None
    returns = sd.returns()
    spike_dir = model["spike_dir"]
    window = min(len(returns), int(model["mean_gap"] * 2) + 50)
    tail = returns[-window:]
    tail_spike_idx = detect_spike_ticks(tail, spike_dir)
    if len(tail_spike_idx):
        ticks_since_last = int(len(tail) - 1 - tail_spike_idx[-1])
    else:
        ticks_since_last = int(model.get("ticks_since_last", model["mean_gap"]))

    hazard_ref = boom_crash_hazard(model, ticks_since_last, duration_ticks=5)
    bg_dir  = -spike_dir     # background drifts opposite the spike direction
    safety  = 1.0 - hazard_ref
    bg_signal = float(np.clip(bg_dir * safety, -1.0, 1.0))

    return {
        "bc_active":           True,
        "bc_spike_dir":        spike_dir,
        "bc_bg_signal":        bg_signal,
        "bc_hazard":           hazard_ref,
        "bc_ticks_since_last": ticks_since_last,
        "bc_mean_gap":         model["mean_gap"],
        "bc_n_spikes_seen":    model["n_spikes_seen"],
    }
...
def fit_symbol_models(sd) -> SymbolModels:
    ...
    # No-op for R_/1HZ symbols; fits the spike-hazard profile for BOOM/CRASH.
    models.boom_crash = fit_boom_crash_model(sd)
...
def compute_features(sd, models, returns_window_dict):
    ...
    # BOOM/CRASH spike-hazard layer — no-op (all zeros) for R_/1HZ symbols
    bc_feats = compute_boom_crash_features(sd, getattr(models, "boom_crash", None))
    bc_bg_signal        = bc_feats["bc_bg_signal"]        if bc_feats else 0.0
    bc_hazard           = bc_feats["bc_hazard"]           if bc_feats else 0.0
    bc_active            = bool(bc_feats)
    bc_ticks_since_last = bc_feats["bc_ticks_since_last"] if bc_feats else 0
    bc_mean_gap          = bc_feats["bc_mean_gap"]         if bc_feats else 0.0
    ...
    _layer_votes = [
        ...
        bc_bg_signal,                    # BOOM/CRASH background-drift signal
    ]
    ...
    return {
        ...
        "jump_dir":     jump_dir,
        "post_jump":    post_jump,
        # BOOM/CRASH spike-hazard layer (all zero/inactive for R_/1HZ symbols)
        "bc_active":            bc_active,
        "bc_bg_signal":         bc_bg_signal,
        "bc_hazard":            bc_hazard,
        "bc_ticks_since_last":  bc_ticks_since_last,
        "bc_mean_gap":          bc_mean_gap,
...
def bayesian_fusion(features):
    ...
    evidence += [
        ...
        # BOOM/CRASH background-drift layer. Zero for R_/1HZ symbols (bc_bg_signal
        # is 0.0 there), so this term is a pure no-op outside BOOM/CRASH. Given a
        # strong default weight because, unlike the other layers, it reflects the
        # index's actual engineered mechanic rather than a statistical inference.
        (features["bc_bg_signal"],                              W("bc_drift", 1.8)),
    ]
    ...
    # ── BOOM/CRASH: hazard-based confidence dampening ──────────────────────
    # As the "ticks since last spike" count approaches the expected interval,
    # the background-drift thesis becomes less trustworthy (a spike could
    # land mid-contract and overwhelm the drift). Dampen log_odds toward zero
    # as hazard rises past BC_HAZARD_DAMPEN_FROM — this is a soft pre-warning;
    # the hard BC_MAX_HAZARD_GATE check (using the actual chosen duration)
    # happens later in the entry pipeline and can reject the trade outright.
    bc_hazard = float(features.get("bc_hazard", 0.0))
    if bc_hazard > BC_HAZARD_DAMPEN_FROM:
        damp = float(np.clip((bc_hazard - BC_HAZARD_DAMPEN_FROM) * 1.5, 0.0, 0.9))
        log_odds *= (1.0 - damp)
    ...
    # NOTE: skipped for BOOM/CRASH symbols — a BOOM index trading mostly PUT
    # (and CRASH mostly CALL) between spikes is the correct, engineered
    # behavior of the index, not a structural bias to correct away.
    if not features.get("bc_active", False):
...
def online_update_layer_weights(models: SymbolModels, feats: dict,
    ...
    layer_signals = {
        ...
        "bc_drift":   feats.get("bc_bg_signal",   0),
    }
...
async def deep_startup_calibration(state, symbol_data, symbols):
    ...
    for s in symbols:
        sd = symbol_data[s]
        n  = len(sd.ticks)
        fam = "1HZ" if "1HZ" in s else ("BC " if is_boom_crash_symbol(s) else "R_ ")
        ...
        if report["best_models"] is not None and report["best_models"].fitted \
                and report["best_models"].boom_crash is not None:
            bc = report["best_models"].boom_crash
            print(f"  BC spike model    : {bc['n_spikes_seen']} spikes seen, "
                  f"mean_gap={bc['mean_gap']:.0f} ticks (nominal={bc['nominal_interval']}), "
                  f"std_gap={bc['std_gap']:.0f}, bg_drift={bc['bg_drift']:+.6f}/tick, "
                  f"ticks_since_last={bc['ticks_since_last']}")
...
async def main():
    ...
    # --- BOOM/CRASH 500 & 1000 spike indices ---
    bc_symbols = []
    for attempt in range(1, 4):
        bc_symbols = await fetch_boom_crash_symbols(client)
        if bc_symbols:
            break
        print(f"[main] No BOOM/CRASH symbols on attempt {attempt}/3, retrying in 3s...")
        await asyncio.sleep(3)
    if not bc_symbols:
        print("[main] WARNING: no BOOM/CRASH symbols available - proceeding without them.")

    symbols = r_symbols + hz_symbols + bc_symbols
    print(f"\nFull tradable universe ({len(symbols)} symbols):")
    print(f"  R_ ({len(r_symbols)}): {r_symbols}")
    print(f"  1HZ top-3 ({len(hz_symbols)}): {hz_symbols}")
    print(f"  BOOM/CRASH ({len(bc_symbols)}): {bc_symbols}")

    # build SymbolData with correct tick_dt / buffer size per family
    symbol_data = {}
    for s in r_symbols:
    ...
    for s in bc_symbols:
        # Larger buffer than R_/1HZ: the spike-gap distribution needs enough
        # history to see multiple spikes, especially for the 1000-family.
        symbol_data[s] = SymbolData(s, maxlen=BC_MAXLEN, tick_dt=2.0)

    print(f"Bootstrapping tick history for all symbols "
          f"(target: {HISTORY_BOOTSTRAP_COUNT} ticks each, "
          f"{BC_HISTORY_BOOTSTRAP_COUNT} for BOOM/CRASH)...")
    for s in symbols:
        count = BC_HISTORY_BOOTSTRAP_COUNT if s in bc_symbols else HISTORY_BOOTSTRAP_COUNT
        history = await fetch_history(client, s, count=count)
    ...
    while True:
        ...
        if state.recovery_step > 0:
            ...
            # ── BOOM/CRASH hazard gate (recovery path) ──────────────────────
            if feats.get("bc_active", False):
                bc_model = state.model_cache[rec_sym].boom_crash
                haz = boom_crash_hazard(bc_model, feats["bc_ticks_since_last"], duration)
                if haz > BC_MAX_HAZARD_GATE:
                    print(f"[BC-Hazard/Recovery] {rec_sym} skipped — "
                          f"P(spike within {duration} ticks)={haz:.2f} > {BC_MAX_HAZARD_GATE}")
                    continue
        ...
        # ── BOOM/CRASH hazard gate ───────────────────────────────────────────
        # Recompute hazard precisely for the ACTUAL chosen contract duration
        # (compute_features only used a rough 5-tick proxy). If a spike is
        # too likely to land inside this specific contract window, skip —
        # no amount of background-drift edge survives an unresolved spike.
        if feats.get("bc_active", False):
            bc_model = state.model_cache[symbol].boom_crash
            haz = boom_crash_hazard(bc_model, feats["bc_ticks_since_last"], duration)
            if haz > BC_MAX_HAZARD_GATE:
                print(f"[BC-Hazard] {symbol} skipped — P(spike within {duration} ticks)="
                      f"{haz:.2f} > {BC_MAX_HAZARD_GATE} "
                      f"(ticks_since_last={feats['bc_ticks_since_last']}, "
                      f"mean_gap={feats['bc_mean_gap']:.0f})")
                continue
